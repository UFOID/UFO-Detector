...
DetectorState::DetectorState(QObject *parent, Recorder *rec) : QObject(parent), tracker(0.2, 0.5, 60.0, 15, 15) {
    ...
    connect(this, SIGNAL(foundDetectionResult(DetectorState::DetectionResult)), this,
            SLOT(handleResult(DetectorState::DetectionResult)));
...
void DetectorState::handleResult(DetectorState::DetectionResult result) {
    recorder->stopRecording(map_result[result].willSaveVideo, std::chrono::duration_cast<std::chrono::milliseconds>(
